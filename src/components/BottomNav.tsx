...
import { CreditCard, Zap, Home, ShieldAlert, Menu as MenuIcon } from 'lucide-react';
...
const BOTTOM_NAV_ITEMS = [
  { id: 'service-charge', label: 'সার্ভিস চার্জ', icon: CreditCard, view: 'service-charge' },
  { id: 'desco', label: 'ডেসকো', icon: Zap, view: 'desco' },
  { id: 'home', label: 'হোম', icon: Home, view: 'home' },
  { id: 'emergency', label: 'জরুরী', icon: ShieldAlert, view: 'emergency' },
  { id: 'menu', label: 'মেনু', icon: MenuIcon, view: 'menu' },
];

const BottomNav: React.FC<BottomNavProps> = React.memo(({ activePage, setActivePage }) => {
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 pb-0 pointer-events-none">
      <div className="max-w-md mx-auto relative pointer-events-auto">
        ...
        {BOTTOM_NAV_ITEMS.map((item) => {
          const isActive = activePage === item.view;
          const isHome = item.id === 'home';

          return (
            <button
              key={item.id}
              onClick={() => setActivePage(item.view as Page)}
